...
#define T1H  800L    // Width of a 1 bit in ns
#define T1L  200L    // Width of a 1 bit in ns

#define T0H  250L    // Width of a 0 bit in ns
#define T0L  850L    // Width of a 0 bit in ns

#define RES 6000L    // Width of the low gap between bits to cause a frame to latch
...
uint8_t effectRelease = 0, randomEnable = 0;
uint32_t sysTime = 0;
uint8_t bSwitched = 0;
uint8_t effectId = 0;
...
static inline void __attribute__ ((always_inline)) sendBitX8_B( uint8_t bitx8 ) {
  ...
  asm volatile (                        // The below asm code has been modified to match TM1914A timing
    ...
    "out %[port], %[onBits]  \n\t"        // 1st step - all bits high

    ".rept %[T0HCycles] \n\t"               // Execute NOPs to delay exactly the specified T0H number of cycles
    "nop \n\t"
    ".endr \n\t"
...
void setup() {
  ...
  PORTD = 0x00;   // Set all row pins to high (signal line of TM1914A is normally high)
  DDRC = 0x3f;    // Set all row pins to output
  PORTC = 0x00;   // Set all row pins to high (signal line of TM1914A is normally high)
  DDRB = 0x03;    // Set all row pins to output
  PORTB = 0x00;   // Set all row pins to high (signal line of TM1914A is normally high)
...
void doEffects(uint8_t id) {
  ...
  switch (id) {
    case 0:
      rainbow((uint8_t)255 / PIXELS);
      delay((uint8_t)(masterDelay / 4));
      showArray();
      break;
    case 1:
      shine1(masterDelay);
    ...
    case 10:
      rainbow((uint8_t)(255 / PIXELS));
      parallel_1_reverse(masterDelay);
      break;
    case 11:
      shine8_reverse(masterDelay);
      break;
    case 12:
      shine8(masterDelay);
      break;
    ...
    case 14:
      rainbow((uint8_t)(255 / PIXELS));
      parallel_1(masterDelay);
      //if (!randomEnable) randomEnable = 1;
      break;
    default:
      break;
  }
