...
void shine2()
{
  static uint8_t lastPos, lastSeq;
  static uint8_t h, hIndex;
  ...
  if (!(count % NUM_LEDS)) newHue = mainColor[++hIndex % 11];
  if (h < newHue) h += 3;
  if (h > newHue) h -= 3;
...
void shine2b()
{
  ...
  static uint8_t count1;
  static uint16_t count = NUM_LEDS + 1;
  static uint8_t lastSeq;
  uint8_t c = 0;

  if (lastPos == shinePos) return;
  if (lastSeq != effectSequence) count = NUM_LEDS + 1;
  ...
  if (count >= 0) {
    for (int i = count; i >= 0; i--) {
      if ((c == 0 ) && (i <= NUM_LEDS)) leds[i] = CHSV(0, 0, 200);
      if ((c == 1 ) && (i <= NUM_LEDS)) leds[i] = CHSV(gHue, 255, 120);
      if ((c == 2 ) && (i <= NUM_LEDS)) leds[i] = CHSV(gHue, 255, 60);
      if ((c == 3 ) && (i <= NUM_LEDS)) leds[i] = CHSV(gHue, 255, 60);
      if ((c == 4 ) && (i <= NUM_LEDS)) leds[i] = CHSV(gHue, 255, 120);
      c++;
      if (c >= 5)        c = 0;
    }
  }
...
void shine2ex()
{
  ...
  if (lastSeq != effectSequence) count = 0;
...
void shine3()
{
  ...
  if (count >= 0) {
    for (int i = count; i >= 0; i--) {
      ...
      if ((c == 3) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 120, 240);
      if ((c == 4) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 170, 255);
      if ((c == 5) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 170, 240);
      if ((c == 6) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 255, 150);
      if ((c == 7) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 255, 100);
      if ((c == 8) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 255, 70);
      if ((c == 9) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 255, 55);
      if ((c == 10) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 255, 40);
      c++;
      if (c >= 11)  {
        c = 0;
        h ++;
  ...
  count++;
  //if (!(count % 11)) h = mainColor[++hIndex % 11];
...
void shine3ex()
{
  ...
  if (count >= 0) {
    for (int i = count; i >= 0; i--) {
      if ((c <= 2) && (i <= NUM_LEDS)) leds[i] = CRGB(0, 0, 0);
      if ((c >= 3) && (c < NUM_LEDS) && (i <= NUM_LEDS)) leds[i] = CHSV(h, 255, 255);
      if ((c >= NUM_LEDS) && (i <= NUM_LEDS)) leds[i] = CRGB(0, 0, 0);
      c++;
    }
  }
